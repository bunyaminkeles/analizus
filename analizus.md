...
*Son güncelleme: Temmuz 2026 — Bootstrap CDN kaldırma migration'ı başladı (bkz. §27, `tasks/todo.md`): Faz 1 (`.ax-alert`/`.ax-modal`/`.ax-dropdown`/form control bileşenleri `base.css`'e eklendi) ve Faz 2 (`ax-modal.js`/`ax-dropdown.js` vanilla controller'lar; `base.html` + `success_stories.html`'deki 5 Bootstrap modal `.ax-modal` yapısına taşındı) tamamlandı; Faz 3 (dosya dosya component migration) başladı, `account_delete.html` + `donation_success.html` migrate edildi. Bulgu: Bootstrap utility class'ları (`d-flex`/`text-*`/`fw-*` vb.) component'lerden ayrı, geniş bir yüzey — Faz 4'ten önce ayrıca ele alınacak. Önceki: Mobil PageSpeed performans iyileştirmesi: Google Fonts + Bootstrap CDN CSS deferred yüklemeye alındı, preconnect fazlalığı azaltıldı, navbar logosu 1.17MB→48KB küçültüldü (mobil Performans 58→61, LCP 11.0sn→5.5sn; masaüstü 73→94, LCP 2.1sn→1.2sn). Daha önce: Onboarding adım 2/3 bilgilendirme ekranına çevrildi (checkbox çift tetikleme bug'ı + tamamen yeniden tasarım); referral sistemi keşfedilebilirliği (navbar + hoş geldin e-postası); login açık yönlendirme güvenlik açığı düzeltildi; hesap silme onayı yönlendirmesi (`forum_index`→`home`) düzeltildi; silinmiş hesaba mesaj engeli. Ondan önce: İlan süresi artırıldı (10/20/30 gün) + kullanıcı bilgilendirmesi; Referral (Davet) sistemi: ReferralCode+ReferralUse modelleri (migration 0141), azalan getiri (20/10/5 gün), 3 koşul (e-posta+48h+quiz), IP abuse önleme, rozet kademeleri, +50 rep/davet, /davet/ dashboard.*
