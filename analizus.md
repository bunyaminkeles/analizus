...
### Navigasyon Menüsü (`base.html` — Temmuz 2026, anasayfa/navbar turu Faz 1)
```
...
Pazaryeri            → /market/ (tek isim'e sabitlendi: navbar/footer/job_list.html
                       title-meta-H1/home.html JSON-LD hepsi "Pazaryeri" — eski adlar
                       "Hizmetler Pazarı"/"Analiz Hizmetleri Pazarı" temizlendi; URL
                       değişmedi, yalnızca görünen metin)

Topluluk ▾    YENİ    → Forum (/forum/)
                       Çalışma Odaları (/odalar/) — Analizler'den buraya taşındı
                       İstatistik Arena (home.html#istatistik-arenasi ankrajı)
                       Blog (/blog/) — Kurumsal'dan buraya taşındı
                       (aktif path: /forum/, /odalar/, /blog/ altında is-active)

Proje Talebi         → /proje-talebi/ (değişmedi, üst seviyede kalır)

Kurumsal ▾           → Neden Analizus? — ayrı sayfa DEĞİL, about.html#analizusun-farki
                       ankrajı ("Analizus'un Farkı" bölümüne atlar)
                       AI Çözümler (feature flag: agentic_landing)
                       Hakkımızda + İletişim
```
Kullanıcı dropdown'ındaki puan göstergesi (★) artık tıklanabilir — Arena ankrajına
gider (`onclick` + `stopPropagation`, dropdown toggle'ını tetiklemez).
Mobil drawer: Her üst gruba accordion (yeni `mob-topluluk` dahil). Analizler altında
`/analiz/` tek link (eski 16 ayrı link kaldırıldı).
...
- **Merge öncesi son süpürme turu** (temmuz 2026, devam ediyor) — kaynak prompt `analizus_son_supurme_prompt.md` (`~/Desktop/`, repo'da dosya olarak yok); önceki turların ardından kalan maddeleri kapatmak için başlatıldı. Denetlendiğinde maddelerin önemli bir kısmının **önceki turlarda zaten kapatıldığı** görüldü (bu durum art arda üçüncü kez tekrarlandı — bkz. aşağıdaki genel ders):
  ...
  - **Madde 10 — Testler + Deploy Notu:** `forum/tests.py`'e 8 smoke test eklendi (4 ön-seçim, forum arama boş-durumu, `NoIndexMiddleware` dev/prod ayrımı ×2, Tableau facade ilk yüklemede canlı embed olmaması); 31 test, 30 geçti (1 önceden bilinen ilgisiz hata). Deploy notu (migration `0144-0147`, `?v=` denetimi, Hetzner sırası, merge sonrası hatırlatmalar) `tasks/todo.md`'ye yazıldı — en kritik hatırlatma: `feature_agentic_landing` flag'i migration'da `default=False` geliyor, admin panelden elle açılmazsa `/ai-cozumler/` navbar'da/sitemap'te görünmez.
  - **Madde 11 — Ana sayfa sağ kart görseli:** `.ax-agentic-band-card`'a `agentic-hero(-mobile).webp` eklendi, sol karttaki (`.ax-brand-visual`) `<picture>`+overlay deseniyle birebir aynı yapıda. Görseller sol karttaki `ai-dogrulama` görselleriyle piksel-piksel aynı boyutta (1600×893/800×446) — bu yüzden iki kart grid-stretch'e gerek kalmadan doğal olarak eşit yükseklikte oluştu.
  - **Durum:** 11 maddenin tümü tamamlandı, `main`'e merge edildi. Sonraki oturumda (aynı gün) canlıda 3 ayrı hata raporu geldi, aşağıdaki iki yeni bölümde belgelendi: Tableau facade dayanıklılık turu ve marka görseli kenar yumuşatma turu.

- **Tableau facade dayanıklılık turu** (temmuz 2026, merge sonrası canlı hata raporlarına yanıt) — `templates/forum/tableau_dashboard.html`, üç ayrı bulgu/düzeltme:
  - **Bulgu 1 — kök neden teşhisi:** Kullanıcı "interaktif dashboard çalışmıyor" bildirdi, ekran görüntüsünde poster+buton donmuş görünüyordu. Playwright ile test edilince `tableau.embedding.3.latest.min.js`'in agent sandbox'ından CORS hatasıyla yüklenemediği görüldü — ama aynı istek curl ile (aynı header'larla) sorunsuz 200 + `access-control-allow-origin: *` dönüyordu. Bu, sandbox'a özgü bir ağ artefaktı olarak değerlendirildi (muhtemelen headless-browser bot tespiti), kod hatası değil. Yine de gerçek bir eksik bulundu: script yüklenemezse (ad-blocker, ağ hatası, CORS — her ne sebeple olursa olsun) buton sonsuza kadar "Yükle" yazılı kalıyor, hiç geri bildirim vermiyordu — `onload`/`onerror` + 8sn timeout ile "Yüklenemedi, tekrar dene" durumuna dönecek şekilde düzeltildi (`736867b`).
  - **Bulgu 2 — Tableau Public'in kendi kesintisi:** Kullanıcı "502 Bad Gateway"/"504 Gateway Time-out" ekran görüntüsü paylaştı. Doğrudan `public.tableau.com`'un kendi view URL'lerine curl atılınca (siteye hiç uğramadan) dördü de 504/timeout döndü — bu, Analizus kodundan bağımsız, Tableau Public'in ücretsiz katmanının geçici bir kesintisiydi, birkaç dakika sonra kendiliğinden düzeldi. Kod tarafında düzeltilecek bir şey yoktu.
  - **Bulgu 3 — gerçek UX kusuru + tam state machine:** Yukarıdaki iki bulgunun ışığında facade tamamen yeniden tasarlandı (`b7f8ea6`, sonra `4a839a1`): (a) "Yükle" tıklamasından 15sn içinde `tableau-viz`'in `firstinteractive` event'i gelmezse iframe kaldırılıp poster'a dönülüyor, üzerinde "Tableau Public şu an yavaş yanıt veriyor" + [Tekrar Dene]/[Tableau Public'te Aç] butonları çıkıyor (iframe içeriği cross-origin olduğu için okunamıyor, saf timeout yaklaşımı); (b) buton altına "İlk yükleme 5-10 sn sürebilir" mikro-metni eklendi; (c) sekme değiştirildiğinde önceki sekmenin canlı embed'i kaldırılıp facade'a dönülüyor (aynı anda en fazla 1 canlı `tableau-viz`, her `.tableau-wrap` için nesil sayacı `__axGen` ile stale async callback'ler engelleniyor); (d) **kullanıcı bildirdi:** embed yerleştirilir yerleştirilmez facade hemen kayboluyor ama Tableau yavaşsa yerine hiçbir görsel geri bildirim gelmiyordu (boş kutu) — dönen spinner + "Yükleniyor…" katmanı embed alanının üzerine binecek şekilde eklendi (`firstinteractive` gelince kalkıyor). Tüm senaryolar Render'da Playwright ile doğrulandı (0 istek ilk yüklemede, script enjeksiyonu, tek-canlı-iframe, timeout/hata simülasyonu, yükleme overlay'i).

- **Marka görseli kenar yumuşatma turu** (temmuz 2026) — `/tarama/`, `/analiz/`, `/proje-talebi/`, `/hakkımızda/` (×2) ve ana sayfanın iki "AI çağında iki yol" kartındaki görsellerin sert `border: 1px solid` kenarını "silik" (yumuşak, keskin çizgisiz) bir görünüme çevirme isteğiyle başladı, üç iterasyon gerekti:
  - **Deneme 1 (başarısız):** `static/css/brand_visuals.css`'teki `.ax-brand-visual`'den `border` kaldırılıp auth sayfalarındaki teknikle (`box-shadow: inset ... var(--ax-bg)`) değiştirildi (`404ba4e`). Kullanıcı test etti: fark neredeyse görünmüyordu.
  - **Deneme 2 (başarısız):** box-shadow blur/spread değerleri güçlendirildi (`43a3b94`). Yine görünmedi — **kök neden yanlış teşhis edilmişti:** kaynak fotoğraflar zaten çok koyu (gece temalı), koyu-koyu geçiş neredeyse sıfır kontrast üretiyor, box-shadow tekniğinin kendisi bu görsellerle uyumsuzdu.
...
- **Ana Sayfa Faz 12 — Forum vitrini dönüşümü** (temmuz 2026) — kaynak `analizus_forum_seed_konular.md` + iki Gemini dalgası (`analizus_anasayfa_prompt.md`'den devam); `dev`'den `main`'e merge edildi, **Hetzner production'da tam doğrulandı (30/30 konu canlı)**, Render dev'de 28/30 (aşağıda not).
  - **Sahte içerik temizliği:** `forum/management/commands/remove_generic_seed_topics.py` (yeni, idempotent) — `seed_forum.py`'nin ürettiği 6 jenerik AI konusunu ("Fine-tuning vs RAG" vb., sabit `views=3456-5678` ile) siler; site konumlanmasıyla ("AI'a güvenme, doğrulat") çelişiyordu.
  - **30 yeni konu:** `forum/management/commands/reseed_forum_topics.py` (yeni) — gerçek hesaplardan (mevcut ama önceden hiç birleştirilmemiş persona havuzu: `Arastirmaci_B`, `TezDanismani_Prof`, `figen`, `joseph` vb.) SEO'ya dikkat edilerek yazılmış soru+doğrulanmış-uzman-cevabı çiftleri; `--count N` ile kademeli yayın (idempotent, subject bazlı skip). 3 yeni kategori (`akademik-surec`, `veri-analizi-bi`, `ai-ml-agentic`) `get_or_create` ile her ortamda otomatik kurulur.
  - **`PERSONA_META` + otomatik kullanıcı oluşturma:** `get_user()` artık `User.objects.get_or_create` kullanıyor — persona hesapları tek bir merkezi seed komutundan gelmediği (yıllar içinde farklı oturumlarda organik biriktiler) için ortamlar arası taşınabilirlik şarttı; eksik kullanıcı `PERSONA_META` sözlüğündeki title/account_type ile otomatik kuruluyor.
  - **`CATEGORY_KEYWORD_FALLBACK`:** 7 önceden-var-olan kategori slug'ının (`spss`, `regresyon`, `metodoloji`, `python`, `r-programlama`, `icerik`, `danismanlik`) Hetzner/Render'da farklı olduğu keşfedildi (`spss`→`spss-amos` vb., §26'ya eklendi) — tam slug bulunamazsa title+description'da sırayla anahtar kelime aranıyor (`metodoloji`→`["Metodoloji","Nitel"]` gibi çoklu deneme), o da yoksa konu atlanıyor.
  - **Sitemap tavuk-yumurta düzeltmesi:** `forum/sitemaps.py`'deki `TopicSitemap` eskiden yalnızca `views__gte=1000` konuları listeliyordu — yeni/küratörlü konular trafik almadan sitemap'e giremiyordu. Artık `Q(views__gte=1000) | Q(posts__is_best_answer=True)` — uzman cevaplı (dolayısıyla ince/düşük kaliteli olmayan) konular anında sitemap'e girer, cevapsız gürültü hâlâ dışarıda kalır.
  - **Tarih yayma:** `reseed_forum_topics.py` artık `created_at`'ı (`auto_now_add` olduğu için `.update()` ile geriye dönük) konu başına 1-3 gün aralıklarla geçmişe yayıyor, cevabı sorudan 2-20 saat sonrasına damgalıyor — tek çalıştırmada oluşan konular aynı zaman damgasında kümelenip "toplu içerik dökümü" görünümü vermesin diye.
  - **Vitrin sorgusu:** `forum/views.py`'deki `home()`/`forum_index()` artık ortak `_trending_topics()` yardımcısını kullanıyor — sıralama `-views` yerine `Exists()` ile "uzman cevaplı öncelik, dolmazsa son aktif" (N+1 yok). `_gundem_tartismalar.html`'e "Uzman cevapladı" rozeti (`ax-badge--primary`) + cevap sayısı eklendi; `trend_topics.css`'te mobil taşma için `flex-wrap` düzeltmesi (`?v=0002`).
...
- **Local Docker Ortamı Onarımı + Ana Sayfa İstatistik Bandı Düzeltmesi** (23 Temmuz 2026) — kaynak: kullanıcının local Docker ortamındaki art arda sorun bildirimleri (nginx crash-loop, yanlış DB fallback, beklenmedik admin e-posta patlaması, `/market/` istatistik bandının görünmemesi, ana sayfa sayaçlarındaki `+` işaretleri). 4 commit, `dev`'den `main`'e fast-forward merge edildi:
  - **DB fallback + local/Hetzner ayrışması** (nihai hâli `5e37ed9`): `.env`'de `DATABASE_URL` yorumluydu, web container sessizce SQLite'a düşüyordu; Postgres volume'ünün gerçek şifresi `.env`'deki varsayılanla uyuşmuyordu (`ALTER ROLE` ile düzeltildi); DB ayrıca 107 migration geride kalmıştı (`migrate` ile giderildi). **Kritik ders:** `docker-compose.yml` hem local hem Hetzner'de kullanılan PAYLAŞILAN tek dosya (bu bölümdeki §3 ile doğrulandı — Hetzner de aynı dosyayı `git pull` ile çekip çalıştırıyor). İlk denemede bu dosyaya doğrudan bir `DATABASE_URL` environment override'ı yazılmıştı — bu, Hetzner'in gerçek `.env`'ini sessizce ezme riski taşıyordu (Docker Compose'da `environment:` her zaman `env_file`'dan önceliklidir). Düzeltme: `docker-compose.yml` main ile birebir aynı hale geri döndürüldü; tüm local-only ayarlar (`DATABASE_URL` override + nginx self-signed sertifika volume'leri) gitignore'lu `docker-compose.override.yml`'e taşındı (Compose otomatik yükler, asla commit'lenmez/Hetzner'e ulaşmaz).
  - **nginx crash-loop:** production nginx config'i (`./nginx/conf.d`, Let's Encrypt sertifikası bekliyor) local'de sertifika olmadığı için ~60 saniyede bir çöküyordu. 10 gün önce hazırlanmış ama hiç bağlanmamış `nginx/conf.d.local/local.conf`, `openssl req -x509` ile üretilen self-signed sertifikayla (`.gitignore`'lu `nginx/local-ssl/`) `docker-compose.override.yml` üzerinden devreye alındı.
  - **Yan etki — istenmeyen admin e-posta patlaması:** `Dockerfile`'ın CMD'si her container başlangıcında `deploy.sh`'i çalıştırıyor; `deploy.sh`'in "DB boşsa seed yükle" idempotency kontrolü, DB'ye ilk kez gerçekten bağlanılan restart'ta `Topic` tablosunu boş görüp `load_seed_content`'i tetikledi — bu ~78 sahte kullanıcı oluşturdu (`User.objects.create_user`), her biri `forum/signals.py:578` `post_save` sinyali üzerinden gerçek SMTP ile admin'e bildirim e-postası gönderdi. Kalıcı bir hata değildi (kaynak süreç kendi kendine bitti), tekrar aynı restart'ta tetiklenmez çünkü DB artık boş değil.
