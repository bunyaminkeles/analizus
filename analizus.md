...
- **Bootstrap CDN kaldırma** — sadece grid (`container`/`row`/`col-*`) için kullanılıyor; küçük bir custom CSS grid ile değiştirip bağımlılığı tamamen kaldır. Lighthouse'ta "kullanılmayan CSS" (109 KiB) + "kullanılmayan JS" (164 KiB) bulgusunun kaynağı; **pasif bekliyor**
...
*Son güncelleme: Temmuz 2026 — Mobil PageSpeed performans iyileştirmesi: Google Fonts + Bootstrap CDN CSS deferred yüklemeye alındı, preconnect fazlalığı azaltıldı, navbar logosu 1.17MB→48KB küçültüldü (mobil Performans 58→61, LCP 11.0sn→5.5sn; masaüstü 73→94, LCP 2.1sn→1.2sn). Önceki: Onboarding adım 2/3 bilgilendirme ekranına çevrildi (checkbox çift tetikleme bug'ı + tamamen yeniden tasarım); referral sistemi keşfedilebilirliği (navbar + hoş geldin e-postası); login açık yönlendirme güvenlik açığı düzeltildi; hesap silme onayı yönlendirmesi (`forum_index`→`home`) düzeltildi; silinmiş hesaba mesaj engeli. Daha önce: İlan süresi artırıldı (10/20/30 gün) + kullanıcı bilgilendirmesi; Referral (Davet) sistemi: ReferralCode+ReferralUse modelleri (migration 0141), azalan getiri (20/10/5 gün), 3 koşul (e-posta+48h+quiz), IP abuse önleme, rozet kademeleri, +50 rep/davet, /davet/ dashboard.*
