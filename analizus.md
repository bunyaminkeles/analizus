...
class ProjectRequest:
    ...
    analysis_type: choice      # visualization | ml | statistics | cleaning | timeseries | nlp | literature | verification | agentic | other
    description: TextField
    ...
    # `?type=` GET param'ı HİÇ işlenmiyor — yalnızca `source` okunur, ayrı bir type parametresi
    # eklemeye gerek yok (temmuz 2026'da bu varsayımla iki kez karşılaşıldı, her ikisinde de kullanılmadı)
    ...
    # bibliometrics → `?source=bibliometrics`) `service_promo.html`'e opsiyonel `promo_cta_source`
    # context değişkeni eklenmiş; view context'ine `'promo_cta_source': 'X'` geçirilmezse `tool`
    # varsayılanına düşer (temmuz 2026, `bibliometrics/views.py`).
```
