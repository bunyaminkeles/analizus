...
*Son güncelleme: Haziran 2026 — Profil gizlilik (verilen teklifler/fiyatlar başkasına gizlendi); navbar yeniden düzeni (Proje Talebi bağımsız, Forum Kurumsal altında); forum @mention yeni konu formuna eklendi; forum topic_detail DM stili (compact input, gradient balonlar, sabit yükseklik kaldırıldı); forum index kompakt kartlar + `last_post_at` sıralaması; `django.utils.timezone.utc` → `datetime.timezone.utc` (Django 5.2). Önceki: YÖK Tez/TR Dizin → Proje Talebi entegrasyonu; deploy.sh createcachetable; danışmanlık dönüşüm planı; AI Asistan güvenlik/kalite katmanları; İş ilanı kategorileri bağımsızlaştırıldı; SEO GSC düzeltmeleri; Analytics; Ödeme/sipariş admin; Çalışma odası düzenleme/silme + @mention; Semantic Scholar; WoS parser; DM okundu göstergesi; Karar Ağacı + SVM.*
