...
*Son güncelleme: Temmuz 2026 — Hakkımızda/Neden Analizus birleştirildi (`/neden-biz/` 301 redirect), İletişim baypass.net tarzı bağımsız landing page (`/iletisim/`) oldu; `/gizlilik-politikasi/` daha kapsamlı KVKK metniyle güncellendi (Veri Sorumlusu/İşlenme Amaçları tablosu/Aktarım/Saklama/Madde 11/Çerez), Bootstrap `table-dark` yerine `.ax-kvkk-table`'a taşındı, gövde metni okunabilirliği artırıldı; tarama araçları (YÖK Tez/OpenAlex/TR Dizin/OAI-PMH) S3 saklama süresi 3 günden 7 güne çekilip KVKK metniyle eşitlendi (Hizmetler Pazarı tarafı ayrı görev olarak beklemede); YouTube Transcript nav'a eklenip Render'da denendi, cloud IP engeli teyit edilince `feature_transcript` tekrar kapatıldı (default False); navbar logosuna "Analizus" yazısı eklendi (punto iki kez büyütüldü, `navbar.css?v=0104` cache-bust ile). Önceki: Bootstrap CDN kaldırma migration'ı (bkz. §27, `tasks/todo.md`): Faz 1 (`.ax-alert`/`.ax-modal`/`.ax-dropdown`/form control bileşenleri `base.css`'e eklendi) ve Faz 2 (`ax-modal.js`/`ax-dropdown.js` vanilla controller'lar; `base.html` + `success_stories.html`'deki 5 Bootstrap modal `.ax-modal` yapısına taşındı) tamamlandı; Faz 3 (dosya dosya component migration) başladı, `account_delete.html` + `donation_success.html` migrate edildi. Bulgu: Bootstrap utility class'ları (`d-flex`/`text-*`/`fw-*` vb.) component'lerden ayrı, geniş bir yüzey — Faz 4'ten önce ayrıca ele alınacak. Ondan önce: Mobil PageSpeed performans iyileştirmesi: Google Fonts + Bootstrap CDN CSS deferred yüklemeye alındı, preconnect fazlalığı azaltıldı, navbar logosu 1.17MB→48KB küçültüldü (mobil Performans 58→61, LCP 11.0sn→5.5sn; masaüstü 73→94, LCP 2.1sn→1.2sn). Daha önce: Onboarding adım 2/3 bilgilendirme ekranına çevrildi (checkbox çift tetikleme bug'ı + tamamen yeniden tasarım); referral sistemi keşfedilebilirliği (navbar + hoş geldin e-postası); login açık yönlendirme güvenlik açığı düzeltildi; hesap silme onayı yönlendirmesi (`forum_index`→`home`) düzeltildi; silinmiş hesaba mesaj engeli.*
