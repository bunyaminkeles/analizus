...
Daemon thread içinde analyze → PDF → S3 → email akışını yönetir.
"""
import gc
import threading
import logging
...
def run_makaleanaliz_job(job_id: str) -> None:
    """Makale analizi işini arka planda başlat."""
    ...
    def _run():
        ...
        try:
            ...
            if not records:
                job.mark_failed('TR Dizin verisi bulunamadı. Önce aramayı tamamlayın.')
                return
            ...
            try:
                from makaleanaliz.models import MakaleAnaliz
                MakaleAnaliz.objects.get(id=job_id).mark_failed(str(e))
            except Exception:
                pass
