...
# ═══════════════════════════════════════════════════════════════════════════
# TWITTER/X OTOMATİK PAYLAŞIM SİNYALLERİ
# ═══════════════════════════════════════════════════════════════════════════

from django.conf import settings as django_settings
from .models import DailyTip, FreelanceJob

def share_to_twitter_async(content_type: str, instance):
    """
    Twitter'a asenkron paylaşım yap (thread ile)
    ...
    def _share():
        try:
            ...
            if content_type == 'topic':
                tweet_text = format_topic_tweet(instance, site_url)
            elif content_type == 'daily_tip':
                tweet_text = format_daily_tip_tweet(instance, site_url)
            elif content_type == 'job':
                tweet_text = format_job_tweet(instance, site_url)
            else:
                return

            result = service.post_tweet(tweet_text)
            if result.success:
                logger.info(f"Twitter paylaşımı başarılı: {content_type} #{instance.pk} -> {result.tweet_url}")
            else:
                logger.warning(f"Twitter paylaşımı başarısız: {result.error}")

        except Exception as e:
            logger.error(f"Twitter paylaşım hatası: {e}")

    # Arka planda çalıştır
    thread = threading.Thread(target=_share, daemon=True)
    thread.start()


@receiver(post_save, sender=Topic)
def auto_share_topic_to_twitter(sender, instance, created, **kwargs):
    """Yeni konu oluşturulduğunda Twitter'da paylaş"""
    if created and getattr(django_settings, 'TWITTER_AUTO_SHARE_TOPICS', False):
        share_to_twitter_async('topic', instance)


@receiver(post_save, sender=DailyTip)
def auto_share_daily_tip_to_twitter(sender, instance, created, **kwargs):
    """Yeni günlük ipucu oluşturulduğunda Twitter'da paylaş"""
    if created and getattr(django_settings, 'TWITTER_AUTO_SHARE_DAILY_TIPS', False):
        share_to_twitter_async('daily_tip', instance)


@receiver(post_save, sender=FreelanceJob)
def auto_share_job_to_twitter(sender, instance, created, **kwargs):
    """Yeni iş ilanı oluşturulduğunda Twitter'da paylaş"""
    if created and getattr(django_settings, 'TWITTER_AUTO_SHARE_JOBS', False):
        share_to_twitter_async('job', instance)
