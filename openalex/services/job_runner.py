...
def run_scraping_job(job_id):
    """Background thread'de OpenAlex scraping job çalıştır."""
...
def send_demo_email(job):
    ...
    subject = f"OpenAlex Arama - Demo Sonuçlar: {job.get_query_summary()}"

    demo_txt = _generate_alex_results_txt(job.demo_results, job, is_demo=True)

    body_lines = [
        f"Merhaba {user.first_name or user.username},\n",
        f"OpenAlex aracıyla yaptığınız arama sonuçları hazırlanmıştır.\n",
        f"Sorgu: {job.get_query_summary()}",
        f"Toplam Bulunan Sonuç: {job.total_results}",
    ...
    total_price = AlexOrder.calculate_price(job.total_results)
    ...
    body_lines.append(f"  Açıklama     : OpenAlex - {user.username}")
    body_lines.append(f"")
    ...
    body_lines.append(f"  {site_url}/openalex/siparis/{job.id}/")
    body_lines.append(f"")
    ...
    try:
        ...
        email.attach(
            f"openalex_demo_{len(job.demo_results)}_sonuc.txt",
            demo_txt,
            'text/plain',
        )
        email.send()
        ...
        logger.info(f"OpenAlex demo email gönderildi: {to_email}")
        return True
    except Exception as e:
        logger.error(f"OpenAlex demo email gönderilemedi: {e}")
        return False
...
def send_order_results_email(order):
    ...
    txt_content = _generate_alex_results_txt(results_to_send, job, is_demo=False)

    s3_key = f"openalex/orders/{order.id}.txt"
    ...
    try:
        ...
        email.attach(
            f"openalex_{len(results_to_send)}_sonuc.txt",
            txt_content,
            'text/plain',
        )
        email.send()
