"""
Tez & Makale Analizi PDF Rapor Oluşturucu.
Bibliometrics pdf_builder.py ile aynı altyapı, tek PDF (tüm analizler, ücretsiz).
"""
import io
import os
import logging
...
def _draw_header(c, width, height, title, page_num=None, total_pages=None):
    ...
    c.drawString(22, height - 42, 'Tez & Makale Analizi — Analizus')
...
def _draw_cover(c, width, height, total_records: int, query_summary: str):
    ...
    c.drawCentredString(width / 2, height - 118, 'Tez & Makale Analizi Raporu')
...
def build_pdf(figures: list, total_records: int = 0, query_summary: str = '') -> bytes:
    ...
    Tüm figürleri kapak + sayfa sayfa PDF'e yazar.
    """
    ...
    for i, (title, fig) in enumerate(figures, start=1):
        try:
            ...
            logger.warning(f'[tezanaliz pdf] Grafik sayfası hatası [{title}]: {e}')
        c.showPage()
