...
# Türkçe stopword listesi (NLTK gerekmez)
TR_STOPWORDS = {
    've', 'ile', 'bir', 'bu', 'da', 'de', 'ki', 'mi', 'mu', 'mü', 'mı',
    'bu', 'şu', 'o', 'ben', 'sen', 'biz', 'siz', 'onlar', 'için', 'gibi',
    'kadar', 'daha', 'çok', 'az', 'en', 'her', 'hiç', 'ne', 'nasıl',
    'neden', 'hangi', 'hem', 'ya', 'veya', 'ama', 'fakat', 'çünkü', 'eğer',
    'ise', 'üzerine', 'üzerinde', 'göre', 'sonra', 'önce', 'yani', 'sadece',
    'bile', 'artık', 'hala', 'zaten', 'belki', 'sanki', 'değil', 'var',
    'olan', 'olarak', 'olan', 'edilmiş', 'yapılan', 'yapılmış', 'edilen',
...
def keyword_tfidf_chart(records: list[dict]) -> tuple[str, plt.Figure] | None:
    ...
    bars = ax.barh(top_words[::-1], top_scores[::-1], color=PALETTE[1])
    ax.set_title('TF-IDF Anahtar Kelimeler (Başlık + Özet)')
...
def lda_topics_chart(records: list[dict]) -> tuple[str, plt.Figure] | None:
    ...
    try:
        ...
        vec = CountVectorizer(
            ...
            stop_words=list(TR_STOPWORDS),
            min_df=2,
            token_pattern=r'\b[a-zA-ZğüşıöçĞÜŞİÖÇ]{4,}\b',
        )
...
# ─── 7. Dizin/Konu Kelime Bulutu ─────────────────────────────────────────────

def subject_wordcloud(records: list[dict]) -> tuple[str, plt.Figure] | None:
    word_freq: Counter = Counter()

    for r in records:
        for field in ('konu', 'dizin'):
            val = (r.get(field) or '').strip()
            if val:
                # Virgül veya noktalı virgülle ayrılmış terimler
                terms = re.split(r'[,;]+', val)
                for term in terms:
                    term = term.strip()
                    if len(term) > 2:
                        word_freq[term.lower()] += 1

    if len(word_freq) < 5:
        logger.info('[tezanaliz] Yeterli konu/dizin terimi yok, wordcloud atlanıyor.')
        return None
    ...
    try:
        ...
        ax.axis('off')
        ax.set_title('Konu / Dizin Terimleri Kelime Bulutu')
        fig.tight_layout()
        return ('Konu & Dizin Bulutu', fig)
    except Exception as e:
...
def compute_similar_theses(records: list[dict], query_text: str, top_n: int = 10) -> list[dict]:
    ...
    try:
        ...
        corpus = []
        for r in records:
            parts = [r.get('title', ''), r.get('title_tr', ''), r.get('abstract_tr', '')]
