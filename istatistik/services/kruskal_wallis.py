...
def _conclusion(p, group_col, dep_col, posthoc) -> str:
    if float(p) >= 0.05:
        return (gettext('Gruplar arasında %(dep_col)s açısından istatistiksel olarak anlamlı bir fark bulunmamaktadır (%(p)s).') % {'dep_col': dep_col, 'p': ('p < .001' if p < 0.001 else f'p = {p:.4f}')})
