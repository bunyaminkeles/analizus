"""
Destek Vektör Makinesi (SVM) Sınıflandırması
...
def analyze(df, target_col: str, feature_cols: list,
    ...
    kernel = kernel if kernel in ('linear', 'rbf', 'poly') else 'rbf'
    C = max(0.01, min(100.0, float(C)))

    clf = SVC(kernel=kernel, C=C, random_state=42, probability=False)
    ...
    perm = permutation_importance(clf, X_test_sc, y_test, n_repeats=10, random_state=42, scoring='accuracy')
